...
sequenceDiagram
    ...
    API-->>FE: 返回 run_id
    W->>DB: 拉取任务
    W->>W: 执行实时 fast-first 主链（direct candidate 优先 + 页面补充探索）
    W->>DB: 结果落库 + Artifact
    loop 非终态时
...
- 当前代码中的 CVE 主链仍是 `resolve_seeds -> plan_frontier -> fetch_page -> analyze_page -> download_patches -> finalize_run` 的线性 fast-first 运行链。
- `plan_frontier` 当前会先从全部 seed references 中提取 direct patch/commit 候选，再对剩余普通页面做最小一跳规划。
- 页面抓取失败当前按局部失败处理，只要仍有 direct candidate 或其他成功页面，run 仍会继续推进。
- `graph run` 和 `LLM fallback` 仍属于后续演进方向，不应表述为当前已落地事实。
...
### v1.3 - 2026-04-16
- 同步 CVE fast-first 主链已支持 seed 直达 patch/commit 优先消费。
- 同步页面抓取改为局部容错，不再被单页异常直接拖死整次运行。
