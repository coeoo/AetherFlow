...
- `CVE` 工作台：当前 active run 进入终态后，额外刷新最近运行列表一次，保证顶部摘要与历史列表一致。
...
- 当前前端工作台与详情页的真实路由已经收口为 `/patch` 与 `/patch/runs/:runId`。
- `/cve` 只允许继续作为历史文档中的产品语义，不再作为当前页面实现路径。
...
### v1.1 - 2026-04-16
- 同步 CVE 工作台 active run 终态后会刷新最近运行列表一次。
- 补充“摘要与历史状态一致性”作为当前前端轮询策略的一部分。
