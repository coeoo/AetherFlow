...
```mermaid
flowchart TB
    HOME["/ 平台首页"] --> CVE["/patch CVE Patch 搜索工作台"]
    HOME --> ANN["/announcements 安全公告工作台"]

    CVE --> CVE_RUN["/patch/runs/run_id\nPatch 搜索详情"]
...
| 当前页面 | 触发动作 | 跳转页面 | 说明 |
|----------|----------|----------|------|
| `/` | 点击 Patch 搜索卡片 | `/patch` | 从首页进入 Patch Agent 主链 |
| `/` | 点击公告卡片 | `/announcements` | 从首页进入公告主链 |
| `/` | 点击最近任务 | 场景详情页 | 按 `scene_name + result_ref` 决定落点 |
| `/patch` | 创建 run | `/patch` 当前页刷新 | 工作台展示运行摘要、预算和结论 |
| `/patch` | 点击查看详情 | `/patch/runs/{run_id}` | 进入搜索图与证据详情页 |
...
- 将 Patch 搜索主路径从 `/cve` 同步为 `/patch`
- 将详情页主路径从 `/cve/runs/{run_id}` 同步为 `/patch/runs/{run_id}`

### v2.0 - 2026-04-20

- 当时将 `/cve` 与 `/cve/runs/{run_id}` 定义为 Patch 搜索主路径（现已被 `/patch` 路由取代）
- 将一级导航统一为 `Patch 搜索`
- 将结果页定位更新为“搜索图与证据详情页”
